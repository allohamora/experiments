...
export const performanceTest = async (target) => {
  ...
  return { time, memory: { start: formatMemory(memoryStart), end: formatMemory(memoryEnd), diff: formatMemory(memoryDiff) }, result };
};
...
const averageMemory = (memoryValues) => {
  const { startValues, endValues, diffValues } = memoryValues.reduce((state, { start, end, diff }) => {
